...
test("generate rejects invalid EC level", function()
  local qr = require("qr")
  assert_error(function()
    qr.generate("test", { ec_level = 5 })
  end, "invalid EC level should throw")
end)
...
test("render rejects invalid quiet zone", function()
  local qr = require("qr")
  assert_error(function()
    qr.render("test", { quiet_zone = -1 })
  end, "invalid quiet zone should throw")
end)
...
if has_nvim_api then
  ...
  test("setup rejects invalid config", function()
    reset_modules()
    local qr = require("qr")

    assert_error(function()
      qr.setup({ ec_level = 0 })
    end, "invalid ec_level should throw")
    assert_error(function()
      qr.setup({ quiet_zone = 1.5 })
    end, "fractional quiet_zone should throw")
    assert_error(function()
      qr.setup({ invert = "yes" })
    end, "non-boolean invert should throw")
    assert_error(function()
      qr.setup({ keymap = true })
    end, "non-string keymap should throw")
    assert_error(function()
      qr.setup({ max_title_length = -1 })
    end, "negative max_title_length should throw")
  end)
